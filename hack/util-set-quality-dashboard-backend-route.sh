...
yq e -i "(.configMapGenerator[].literals[] | select(. == \"*BACKEND_ROUTE*\")) = \"BACKEND_ROUTE=http://quality-backend-route-quality-dashboard.apps.$CLUSTER_URL_HOST\"" "$KUSTOMIZATION_FILE"
