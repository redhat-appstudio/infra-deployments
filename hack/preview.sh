...
[[ -n "${JVM_BUILD_SERVICE_SIDECAR_IMAGE_REPO}" && ${JVM_BUILD_SERVICE_SIDECAR_IMAGE_TAG} ]] && yq -i e "(.spec.template.spec.containers[].env[] | select(.name==\"JVM_BUILD_SERVICE_SIDECAR_IMAGE\")) |=.value=\"${JVM_BUILD_SERVICE_SIDECAR_IMAGE_REPO_REPO}:${JVM_BUILD_SERVICE_SIDECAR_IMAGE_REPO_TAG}\"" $ROOT/components/build/jvm-build-service/operator-images.yaml
