...
[[ -n "${JVM_BUILD_SERVICE_PR_OWNER}" && "${JVM_BUILD_SERVICE_PR_SHA}" ]] && yq -i e "(.resources[] | select(. ==\"*github.com/redhat-appstudio/jvm-build-service*\")) |= \"https://github.com/${JVM_BUILD_SERVICE_PR_OWNER}/jvm-build-service/config/default?ref=${JVM_BUILD_SERVICE_PR_SHA}\"" $ROOT/components/build/jvm-build-service/kustomization.yaml
