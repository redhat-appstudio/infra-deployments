...
[[ -n "${JVM_BUILD_SERVICE_CACHE_IMAGE}" && ${JVM_BUILD_SERVICE_REQPROCESSOR_IMAGE} ]] && patch=$(yq "(.patches[] | select(.target.kind == \"Deployment\")).patch" $ROOT/components/jvm-build-service/kustomization.yaml | yq e "select(.[].path == \"/spec/template/spec/containers/0/env\") | .[].value |= . + [{\"name\" : \"JVM_BUILD_SERVICE_CACHE_IMAGE\", \"value\": \"${JVM_BUILD_SERVICE_CACHE_IMAGE}\"}, {\"name\": \"JVM_BUILD_SERVICE_REQPROCESSOR_IMAGE\", \"value\": \"${JVM_BUILD_SERVICE_REQPROCESSOR_IMAGE}\"}] | (.[].value[] | select(.name == \"IMAGE_TAG\")) |= .value = \"\"") yq -i e "(.patches[] | select(.target.kind == \"Deployment\")).patch |= strenv(patch)" $ROOT/components/jvm-build-service/kustomization.yaml
