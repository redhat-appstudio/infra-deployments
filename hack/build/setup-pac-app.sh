...
payload="{ \"iss\": $GITHUB_APP_ID, \"iat\": ${now}, \"exp\": $((now+10)) }"

if ! oc get -n pipelines-as-code secret pipelines-as-code-secret &>/dev/null; then
  oc create namespace pipelines-as-code 2>/dev/null
  webhook_secret=$(openssl rand -hex 20)
  token=$(sign rs256 "$payload" "$(echo $GITHUB_APP_PRIVATE_KEY | base64 -d)")
  oc -n pipelines-as-code create secret generic pipelines-as-code-secret \
        --from-literal github-private-key="$(echo $GITHUB_APP_PRIVATE_KEY | base64 -d)" \
  ...
  curl \
    ...
    -d "{\"content_type\":\"json\",\"insecure_ssl\":\"1\",\"secret\":\"$webhook_secret\",\"url\":\"$webhook_url\"}"
fi
