...
yq e ".spec.template.spec.source.repoURL=\"$GITURL\"" $OVERLAYDIR/repo-overlay.yaml -i
yq e ".spec.template.spec.source.targetRevision=\"$BRANCH\"" $OVERLAYDIR/repo-overlay.yaml -i
