...
configure_compute_workspace() {
  ...
  if [[ -z "$(kubectl get synctargets.workload.kcp.dev ${SYNC_TARGET} --kubeconfig ${KCP_KUBECONFIG} 2>/dev/null)" ]]; then
    ...
    KUBECONFIG=${KCP_KUBECONFIG} kubectl kcp workload sync ${SYNC_TARGET} --syncer-image ghcr.io/kcp-dev/kcp/syncer:${KCP_VERSION:-"main"} --resources=services,routes.route.openshift.io -o /tmp/${SYNC_TARGET}-syncer.yaml
