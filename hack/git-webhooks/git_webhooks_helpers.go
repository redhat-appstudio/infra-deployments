...
func doesRepoHaveSecrets(repo Repository) bool {
	return repo.Spec.GitProvider.WebhookSecret != RepoSecret{} && repo.Spec.GitProvider.PacsSecret != RepoSecret{}
}
...
func getSpecialExternalRepos() ([]Repository, error) {
	...
	fmt.Println("Searching for Repository resources with external Git provider URLs '" + gitLabComURL + "' and '" + gitHubComURL + "'...")
	...
	for scanner.Scan() {
		...
		if !doesRepoHaveSecrets(repo) {
			if repo.Spec.GitProvider.URL != gitHubComURL {
				fmt.Printf("Warning: Repository %s does not have webhook and PACs secrets and has a GitLab URL. Skipping...\n", repo.Metadata.Name)
			}
			continue
		}
