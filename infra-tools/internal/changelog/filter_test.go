...
// --- AllNonMergeCommitsFilter ---

func TestAllNonMergeCommitsFilter_KeepsAllNonMerge(t *testing.T) {
	g := NewWithT(t)
	commits := []CommitInfo{
		makeCI("chore: whatever", false),
		makeCI("Implement feature without prefix", false),
		makeCI("Merge pull request #1", true),
		makeCI("fix: a fix", false),
	}
	result := AllNonMergeCommitsFilter(commits)
	g.Expect(result).To(HaveLen(3))
}

func TestAllNonMergeCommitsFilter_ExcludesMergeOnly(t *testing.T) {
	g := NewWithT(t)
	commits := []CommitInfo{
		makeCI("Merge pull request #1 from foo/bar", true),
		makeCI("Merge branch 'main'", true),
	}
	result := AllNonMergeCommitsFilter(commits)
	g.Expect(result).To(BeEmpty())
}
