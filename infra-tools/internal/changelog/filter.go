...
// AllNonMergeCommitsFilter returns all commits that are not merge commits.
// Use this for services whose commit messages do not follow conventional
// commits, so no relevant changes are hidden.
func AllNonMergeCommitsFilter(commits []CommitInfo) []CommitInfo {
	var out []CommitInfo
	for _, c := range commits {
		if !c.IsMerge {
			out = append(out, c)
