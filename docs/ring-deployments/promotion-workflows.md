...
Kargo copies `base/` into ring-0's `base-snapshot/`, then copies each ring's `base-snapshot/` forward to the next ring sequentially. A change never skips a ring — ring-1 gets the content of ring-0's `base-snapshot/`, ring-2 gets ring-1's, and so on. This means back-to-back Tier 1 changes can both be in flight across different rings simultaneously.
...
Never put image tags or digest references inside `base/` or `base-snapshot/`. Those belong in the `images:` block of `rings/ring-N/base/kustomization.yaml` (outside `base-snapshot/`). See [architecture.md §3.3](architecture.md#33-ci-and-artifact-ingestion) for why the two channels must write to separate locations.
...
|---------|---------|-----------|
| `image-wh` | Quay registry | `rings/ring-N/base/kustomization.yaml` (`images:` block) |
| `manifest-wh` | `components/{component}/base/` in Git | `rings/ring-N/base/base-snapshot/` |

Because they write to **different locations** within the ring directory, they never conflict. Each generates its own Freight, opens its own PR, and promotes independently.
...
graph TD
    ...
    F1 -->|auto-promote| R0a["ring-0 — writes kustomization.yaml images: block"]
    F2 -->|auto-promote| R0b["ring-0 — writes base-snapshot/"]

    R0a --> R1a["ring-1 — writes kustomization.yaml images: block"]
...
The final rendered manifest (Tier 3 → Tier 2 → base-snapshot + `images:` block) will include both the updated resource and the new image.
...
2. Merge the PR. ArgoCD syncs rings 3-4 to 3.9.3 directly — no Kargo Promotion runs.
