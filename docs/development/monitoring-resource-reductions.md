...
|-----------|------------|----------|--------------------|----------------------|
| **Prometheus** (MonitoringStack) | `components/monitoring/prometheus/development/federation/monitoringstack-dev-resources-patch.yaml` | 2 → **1** | 500m → **200m** | 16Gi → **2Gi** |
...
| **Cluster Observability operator** (Subscription) | `components/monitoring/prometheus/development/federation/observability-operator-dev-resources-patch.yaml` | — | **50m** (limit: **200m**) | **256Mi** (limit: **1Gi**) |
...
> ```
> For Prometheus/OBO, `preview.sh --obo` adds `federation/` to the development kustomization so the MonitoringStack and its patches are included.
